...
export function MediaGallery( {
    ...
    const maxSideImages = Math.max( sideImageFiles.length, sideImageUrls.length, 3 );
    const sideImageRefs = useRef<( HTMLInputElement | null )[]>( Array( maxSideImages ).fill( null ) );
    ...
    const getSideImagePreview = ( index: number ) => {
        if ( sideImageFiles[index] ) return URL.createObjectURL( sideImageFiles[index]! );
        ...
        <div>
            ...
            <div className="relative aspect-square bg-gray-100 rounded-t-lg overflow-hidden">
                {allMedia.length > 0 && allMedia[selectedImage] ? (
                    allMedia[selectedImage]?.includes( ".mp4" ) || getVideoPreview() === allMedia[selectedImage] ? (
            ...
            <div className="p-4 border-t">
                <div className="grid grid-cols-4 gap-2">
                    ...
                    <div
                        className={`relative aspect-square bg-gray-100 rounded-lg overflow-hidden cursor-pointer border-2 ${selectedImage === 0 ? "border-primary" : "border-transparent"
                            }`}
                    ...
                    {( videoFile || videoUrl ) && (
                        <div
                            className={`relative aspect-square bg-gray-100 rounded-lg overflow-hidden cursor-pointer border-2 ${selectedImage === 0 && videoFile
                                    ? "border-primary"
                                    : "border-transparent"
                                }`}
                            onClick={() => onSelectedImageChange( 0 )}
                    ...
                    {Array.from( { length: maxSideImages } ).map( ( _, index ) => (
                        <div
                            key={`side-${index}`}
                            className={`relative aspect-square bg-gray-100 rounded-lg overflow-hidden cursor-pointer border-2 ${selectedImage === index + 1
                                    ? "border-primary"
                                    : "border-transparent"
                                }`}
